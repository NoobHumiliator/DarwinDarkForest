modifier_frostbitten_icicle_thinker = class({})

------------------------------------------------------------------

function modifier_frostbitten_icicle_thinker:IsHidden()
	return true
end

------------------------------------------------------------------

function modifier_frostbitten_icicle_thinker:IsPurgable() 
	return false
end

------------------------------------------------------------------

function modifier_frostbitten_icicle_thinker:OnCreated( kv )
...
function modifier_frostbitten_icicle_thinker:OnIntervalThink()
	if IsServer() then
		...
		for _,enemy in pairs( enemies ) do
			if enemy ~= nil and enemy:IsMagicImmune() == false and enemy:IsInvulnerable() == false then
				...
				enemy:AddNewModifier( self:GetCaster(), self:GetAbility(), "modifier_frostbitten_icicle", { duration = self.freeze_duration } )
			end
		end
